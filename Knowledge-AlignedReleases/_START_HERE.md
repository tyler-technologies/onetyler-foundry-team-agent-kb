...
This file is **the chatbot's first read** for the Aligned Releases domain. It is a **routing guide**,
...
> collection.** It exists so that Aligned Releases content has an obvious home the moment it is
> written, instead of accumulating in `Knowledge-BP-General/`. Until an agent exists,
> Aligned Releases questions are answered by the **General Blueprint Docs Agent** from the files listed
...
|---|---|
| `FAQ-AlignedReleases.md` | **Authored answers with no upstream source** — verbal SME guidance, observed behaviour, corrections upstream owners have not yet made. Empty of entries today. |
...
`Knowledge-BP-General/Docusaurus-AlignedReleases.md` (527 lines) — glossary, key concepts, integration guide, API reference, and an integration checklist.
...
- Release trains, cadence and calendar.
- Version alignment rules across products.
- The release status lifecycle, including what `Planned` means and who may see it.
- The Aligned Releases API and integrating a product with it.
- Release readiness and sign-off expectations.

## What does NOT belong here

- **Deployment pipelines, Harness, CI/CD** → `Knowledge-BP-General/` (`Docusaurus-DevOps.md`).
- **Product registration** → `Knowledge-OpsCenter/` and `Knowledge-BP-General/`.
- **Incident and outage communication** → `Knowledge-StatusPages/`.
- **Uptime commitments and credits** → `Knowledge-SLAs/`.

---

## Cross-domain pointers

- **Deployment, pipelines, environments** → `Knowledge-BP-General/`.
- **Product registration and licensing** → `Knowledge-OpsCenter/`.
- **Outage communication during a release** → `Knowledge-StatusPages/`.
...
2. Never answer a Aligned Releases question from model priors. If neither this corpus nor the
   BP-General files cover it, say so.
3. Entries in `FAQ-AlignedReleases.md` have **no upstream document** — if challenged, say the answer
