...
| **Who gets the release emails / which CRM field is used** | `FAQ-AlignedReleases.md` → *Who receives the release emails…* **The answer is NOT a CRM field** — recipients are the Org Admins recorded in Admin Center. Also explains what an Org Admin is. |
...
| Whether release notifications follow a CRM contact | They do **not**. Admin Center's Org Admin list is the source. A question phrased around CRM fields still belongs to `FAQ-AlignedReleases.md`, not the Ops Center CRM files. |
...
| **"Do I set the stage once?"** / is stage set-and-forget / who sets GA | `FAQ-AlignedReleases.md` → *Is the feature stage a "set it and forget it" label, or does it change over time?* Stage is a **changing** status across a whole workflow; preview stages are **optional** and skippable; **the system sets GA automatically** when the feature is assigned to a release, tied to the Cohort 1 date. |
