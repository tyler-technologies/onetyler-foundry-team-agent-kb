# START HERE — Routing Guide for the Status Pages Knowledge Corpus

This file is **the chatbot's first read** for the Status Pages domain. It is a **routing guide**,
not a tutorial: its job is to pick the right file before answering, and to know what this
corpus does not cover.

Domain: Tyler status pages — public and internal service-status communication: incident lifecycle, component modelling, subscriptions and notifications.
Audience: Tyler operations, support, product and communications staff handling or announcing service events.

> ⚠️ **This corpus is a scaffold — it has no substantive content yet, and no Foundry agent or
> collection.** It exists so that Status Pages content has an obvious home the moment it is
> written, instead of accumulating in `Knowledge-BP-General/`. Until an agent exists,
> Status Pages questions are answered by the **General Blueprint Docs Agent** from the files listed
...
|---|---|
| `FAQ-StatusPages.md` | **Authored answers with no upstream source** — verbal SME guidance, observed behaviour, corrections upstream owners have not yet made. Empty of entries today. |
...
- Which status page serves which audience, and their URLs.
- Incident lifecycle: opening, updating, resolving, post-incident.
- How components and services are modelled on the page.
- Subscriptions and notification channels for customers and staff.
- Who is authorised to post, and the expected tone and cadence of updates.

## What does NOT belong here

- **Uptime percentages, credits, contractual commitments** → `Knowledge-SLAs/`. A status page
  reports what happened; an SLA says what was promised.
- **Incident *response* runbooks and P1 process** → `Knowledge-BP-General/` (`Docusaurus-DevOps.md`).
- **Release announcements** → `Knowledge-AlignedReleases/`.

---

## Cross-domain pointers

- **What was promised, and remedies** → `Knowledge-SLAs/`.
- **Incident response, DR, on-call** → `Knowledge-BP-General/` (`Docusaurus-DevOps.md`).
- **Planned release communication** → `Knowledge-AlignedReleases/`.
...
2. Never answer a Status Pages question from model priors. If neither this corpus nor the
   BP-General files cover it, say so.
3. Entries in `FAQ-StatusPages.md` have **no upstream document** — if challenged, say the answer
